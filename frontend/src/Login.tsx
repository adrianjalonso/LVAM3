...
function FormLogin({
  ...
  const isVisible = visibilidad ? "text" : "password";
  const visibilityIcon = visibilidad ? "visibility_off" : "visibility";
    ...
    <div>
        ...
        <label className="flex flex-col">
          ...
          <div className="flex flex-col ">
            ...
            <div
              ...
              className=" flex w-8 justify-center items-center relative left-[290px] sm:left-[350px] -top-9"
            >
              <span className="material-symbols-outlined text-gray-500">{visibilityIcon}</span>
            </div>
          </div>
        </label>
      </div>
      ...
      <div className="flex flex-col gap-3">
        <button className="flex items-center justify-center gap-3 w-full h-12 px-4 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-200  transition-colors">
          ...
          <span className="text-text-main  font-medium">Entrar com Google</span>
        </button>
        <button className="flex items-center justify-center gap-3 w-full h-12 px-4 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-200  transition-colors">
          <div className="size-6 text-center">
...
function FormCadastro({
  ...
  setIsLogin: React.Dispatch<React.SetStateAction<boolean>>;
  abaLogin: boolean;
  setAbaLogin: React.Dispatch<React.SetStateAction<boolean>>;
}) {
    ...
    <div className="flex gap-2 flex-col">
      ...
      <div>
        <label className="flex flex-col">
          ...
          />
          {errorSenha && (
            <span className="text-red-500 text-sm mt-1">{errorSenha}</span>
          )}
        </label>
      </div>
      <label className="flex flex-row-reverse justify-end gap-2">
        ...
        <input
          onChange={(e) => setCheckboxAcordo(e.target.checked)}
          className="accent-primary rounded  focus:outline-none focus:ring-offset-0"
          type="checkbox"
          value="si"
        />
  ...
  return (
    <>
      <main className="flex flex-col justify-center items-center max-h-full bg-light pt-16 pb-6">
        {mensagemAlert && (
          <div className="bg-amber-100 border border-amber-400 text-amber-700 px-4 py-3 rounded relative mb-4 text-center text-sm  font-medium animate-pulse">
            {mensagemAlert}
          </div>
        )}
        <section className=" flex flex-col rounded-xl shadow-lg pt-4 p-8 w-full  max-w-md bg-white gap-4 mb-4">
          <div className="flex justify-evenly mb-6 transition-colors ">
            <div className="flex justify-center items-center flex-col w-full">
              <p
                onClick={() => setAbaLogin(true)}
                className={` transition-all duration-300 ease-in-out pt-2 pb-2 w-32 py-3 ${
                  abaLogin ? "text-primary" : "text-[#888888]"
                } text-lg font-bold text-center`}
              >
                Entrar
              </p>
              <hr
                className={` border-primary w-full transition-all duration-300  ${
                  abaLogin ? "opacity-100 scale-x-100" : "opacity-0 scale-x-0"
                }`}
              />
            </div>
            <div className="flex justify-center  items-center flex-col w-full">
              <p
                onClick={() => setAbaLogin(false)}
                className={`pt-2 pb-2 w-32 py-3 ${
                  abaLogin ? "text-[#888888]" : "text-primary"
                } text-lg font-bold text-center`}
              >
                Cadastrar-se
              </p>
